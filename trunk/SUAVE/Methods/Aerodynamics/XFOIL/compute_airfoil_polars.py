...
def  read_airfoil_geometry(airfoil):
     ...
     data_block = f.readlines()
     header = data_block[0][0:100].strip()
     ...
     # Loop through each value: append to each column 
     for line_count in range(data_len):
          x_data[line_count] = float(data_block[line_count][2:10].strip())
          y_data[line_count] = float(data_block[line_count][13:21].strip())   
