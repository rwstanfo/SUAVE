...
def append_ctrl_surf_to_wing_segments(wing):
    ...
    for cs in w_cs :
        ...
        sf[0] = w_cs[cs.tag].span_fraction_start
        sf[1] = w_cs[cs.tag].span_fraction_end
        ...
        for i , seg in enumerate(w_seg):
            ...
            if i == 0: # the first segment (root) cannot have any control surfaces 
                ...
                if append_CS == True:
                    ...
                    control_surface.tag                   = w_cs[cs.tag].tag
                    control_surface.function              = w_cs[cs.tag].function
                    control_surface.span_fraction_start   = s_sf[0] 
                    control_surface.span_fraction_end     = s_sf[1]         
                    control_surface.chord_fraction        = w_cs[cs.tag].chord_fraction
                    control_surface.hinge_fraction        = w_cs[cs.tag].hinge_fraction
                    control_surface.deflection            = w_cs[cs.tag].deflection
